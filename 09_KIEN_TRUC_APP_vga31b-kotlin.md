# Kiến trúc app `vga31b-kotlin` (Loan Caculator)
...
App: **Loan Caculator** — package `com.loancaculator`. App liệt kê ứng dụng đã cài trên máy, kiểm tra phiên bản mới trên Play Store, quản lý/gỡ app, xem thông tin thiết bị, lịch sử quét. Viết bằng **Kotlin + Jetpack Compose**, dùng **Hilt** để inject dependency, kiến trúc **single-Activity**.
...
Hệ điều hành khởi chạy app
        │
        ▼
MyApplication.onCreate()          (com.loancaculator.MyApplication : BaseApplication)
